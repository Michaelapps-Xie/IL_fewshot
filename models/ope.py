...
from .temp import FeatureEnhancementModule
...
class OPEModule(nn.Module):
    ...
    def __init__(
        ...
        self.feature_enhancement = FeatureEnhancementModule(4, emb_dim, 4, theta=2.0)
    ...
    def forward(self, f_e, pos_emb, bboxes):
        ...
        if not self.zero_shot:
            ...
            # print("this is shape_or_objectness",shape_or_objectness.shape)
        ...
        if not self.zero_shot:
            ...
            # print("this is appearance",appearance.shape)

        else:
            appearance = None

        if shape_or_objectness is not None:
            shape_or_objectness, enhanced_shape = self.feature_enhancement(shape_or_objectness)

        if appearance is not None:
            appearance, enhanced_appearance = self.feature_enhancement(appearance)
        ...
        # query_pos_emb = query_pos_emb.repeat(1, 2, 1)  # 现在是 [27, 8, 256]
        # print(1)
        if self.num_iterative_steps > 0:
            ...
            # print("this is all_prototypes", all_prototypes.shape)
        else:
...
####################################
        if self.num_iterative_steps > 0:
            memory = f_e.flatten(2).permute(2, 0, 1)
            enhanced_all_prototypes = self.iterative_adaptation(
                enhanced_shape, enhanced_appearance, memory, pos_emb, query_pos_emb
            )
            # print("this is enhanced_all_prototypes", enhanced_all_prototypes.shape)

        else:
            if enhanced_shape is not None and enhanced_appearance is not None:
                enhanced_all_prototypes = (enhanced_shape + enhanced_appearance).unsqueeze(0)
            else:
                enhanced_all_prototypes = (
                    enhanced_shape if enhanced_shape is not None else enhanced_appearance
                ).unsqueeze(0)
            # print("this is enhanced_all_prototypes", enhanced_all_prototypes.shape)
        combined_prototypes = torch.cat((all_prototypes, enhanced_all_prototypes), dim=0)
        ...
        if self.norm_first:
            ...
            if not self.zero_shot:
                tgt = self.norm1(tgt + self.dropout1(self.self_attn(
                    ...
                    key=self.with_emb(appearance, None),
                    value=appearance,
                    attn_mask=tgt_mask,
                    key_padding_mask=tgt_key_padding_mask
                )[0]))
